...
const MovieList = ({type,title,emoji}) => {
  ...
  const fetchMovies = async() =>{
    const response = await fetch(`https://api.themoviedb.org/3/movie/${type}?api_key=0b06ca6ac82e5ea94d7ab72f8f5a9b5a`)
  ...
  useEffect(() => {
    ...
    if(sort.by !== "default") {
      const order = sort.order === "dsc" ? "desc" :"asc";
     sortedMovies = _.orderBy(sortedMovies, [sort.by], [order])
    }
